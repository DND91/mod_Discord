...
public class Chunk
{
    ...
    public Chunk(World world, byte abyte0[], int i, int j, boolean flag)
    {
        this(world, i, j);
        ...
        for (int l = 0; l < 16; l++)
        {
            for (int i1 = 0; i1 < 16; i1++)
            {
                ...
                {
                    int k1 = abyte0[l << 11 | i1 << 7 | j1] & 0xff;

                    if (flag)
                    {
                        k1 = abyte0[l << 12 | i1 << 8 | j1] & 0xff;
                    }
                    ...
                    {
                        storageArrays[l1] = new ExtendedBlockStorage(l1 << 4);
                    }

                    storageArrays[l1].setExtBlockID(l, j1 & 0xf, i1, k1);
                }
            }
        }
    }
    ...
    {
        this(par1World, par2ArrayOfByte, par3, par4, false);
    }
    ...
    public boolean setBlockIDWithMetadata(int par1, int par2, int par3, int par4, int par5)
    {
        ...
        if (k != 0)
        {
            ...
            {
                if (DimensionAPI.blockRemovalMap.containsKey(Integer.valueOf(k)))
                {
                    IBlockRemoval iblockremoval = (IBlockRemoval)DimensionAPI.blockRemovalMap.get(Integer.valueOf(k));
                    iblockremoval.onBlockRemoval(worldObj, l, par2, i1, k, worldObj.getBlockMetadata(l, par2, i1));
                }
                else
                {
        ...
        if (par4 != 0)
        {
            if (!worldObj.isRemote)
            {
                if (DimensionAPI.blockAddedMap.containsKey(Integer.valueOf(par4)))
                {
                    IBlockAdded iblockadded = (IBlockAdded)DimensionAPI.blockAddedMap.get(Integer.valueOf(par4));
                    iblockadded.onBlockAdded(worldObj, l, par2, i1, par4, par5);
                }
                else
                {
    ...
    public int getPrecipitationHeight(int par1, int par2)
    {
        ...
        if (j == -999)
        {
            ...
            for (j = -1; k > 0 && j == -1;)
            {
                ...
                Material material = l == 0 ? Material.air : Block.blocksList[l].blockMaterial;
